...
use gpui::{
    ...
    popup::{PopupNotSupportedError, PopupOptions},
    px, size,
};
...
impl WaylandSurfaceState {
    fn new(
        ...
        if let WindowKind::Popup(options) = &params.kind {
            let Some(parent) = parent.as_ref() else {
                return Err(PopupNotSupportedError.into());
            };

            let positioner = build_popup_positioner(globals, options, options.size);

            let xdg_surface = globals
                .wm_base
                .get_xdg_surface(&surface, &globals.qh, surface.id());

            let xdg_popup = if let Some(parent_xdg_surface) = parent.xdg_surface() {
                ...
                positioner.destroy();
                xdg_popup
            } else if let Some(parent_layer_surface) = parent.layer_surface() {
                // A layer-shell parent takes a null xdg parent and is anchored via the layer surface.
...
// `size` is separate from `options.size` so a resized popup can be re-anchored at its new size.
fn build_popup_positioner(
    ...
    // set_size rejects zero or negative sizes, so clamp.
    positioner.set_size(
        f32::from(size.width).max(1.0) as i32,
        f32::from(size.height).max(1.0) as i32,
    );
    positioner.set_anchor_rect(
        f32::from(options.anchor_rect.origin.x) as i32,
        f32::from(options.anchor_rect.origin.y) as i32,
        f32::from(options.anchor_rect.size.width).max(1.0) as i32,
        f32::from(options.anchor_rect.size.height).max(1.0) as i32,
    );
...
impl WaylandSurfaceState {
    ...
    // Re-anchors a mapped popup at a new size via `xdg_popup.reposition`. Only valid once mapped
    // (after the first configure), otherwise it is a protocol error.
    fn reposition_popup(&self, globals: &Globals, size: Size<Pixels>) {
        ...
        }) = self
        {
            ...
            let positioner = build_popup_positioner(globals, options, size);
...
impl WaylandWindowStatePtr {
    ...
    /// The layer-shell surface backing this window, if it is a layer-shell window. Used to anchor
    /// child popups against a panel/bar.
    ...
    /// The serial and seat to use when grabbing a child popup, derived from the most recent press
    /// event delivered to this window's client.
    ...
    pub fn handle_popup_event(&self, event: xdg_popup::Event) -> bool {
        match event {
            // Only the size is needed. Position is the compositor's, and the following
            // xdg_surface.configure acks and applies the change.
...
impl PlatformWindow for WaylandWindow {
    ...
    fn resize(&mut self, size: Size<Pixels>) {
        ...
        // A popup's placement is the compositor's, so a resize re-runs the positioner via
        // xdg_popup.reposition rather than setting geometry directly, and the Configure reply drives
        // the buffer resize. Before the first configure the popup is unmapped and reposition is
        // illegal, but the initial positioner already carries the size.
        if matches!(state.surface_state, WaylandSurfaceState::Popup(_)) {
            if state.acknowledged_first_configure {
                state.surface_state.reposition_popup(&state.globals, size);
            }
            return;
        }
